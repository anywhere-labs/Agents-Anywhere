...
import { readOptionalJson, removeFile, writeJsonNoClobber } from '../persistence/files.js'
...
export class LoopbackJsonRpcServer {
  ...
  /** Port currently bound for Connector connections, or undefined before start. */
  ...
  async start(): Promise<BridgeEndpoint> {
    ...
    try {
      await this.publishEndpoint(endpoint)
      return endpoint
    } catch (error: unknown) {
  ...
  private async publishEndpoint(endpoint: BridgeEndpoint): Promise<void> {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      if (await writeJsonNoClobber(this.endpointPath, endpoint) === 'created') return
      const existing = await readOptionalJson<unknown>(this.endpointPath)
      if (!isBridgeEndpoint(existing)) {
        throw new Error(`agents-anywhere bridge endpoint already exists but is not a valid descriptor: ${this.endpointPath}`)
      }
      if (processIsAlive(existing.pid)) {
        throw new Error(`agents-anywhere bridge endpoint is already owned by live process ${existing.pid}`)
      }
      const current = await readOptionalJson<unknown>(this.endpointPath)
      if (!sameEndpointOwner(current, existing)) continue
      await removeFile(this.endpointPath)
    }
    throw new Error('agents-anywhere bridge endpoint ownership changed while reclaiming a stale descriptor')
  }
...
function sameEndpointOwner(value: unknown, expected: BridgeEndpoint): boolean {
  return isBridgeEndpoint(value) && value.pid === expected.pid && value.token === expected.token
}
