...
def upgrade() -> None:
    bind = op.get_bind()
    _validate_sequence_ranges(bind)
    session_columns = {
...
def downgrade() -> None:
    ...
    if allocated_ahead is not None:
        raise RuntimeError(
            "cannot downgrade v2.24 while allocated sequence ranges are ahead of "
    ...
    for table_name, column_name in (
        *(("sessions", column_name) for column_name in _SESSION_SEQUENCE_COLUMNS),
        ...
        if out_of_range is not None:
            raise RuntimeError(
                f"cannot downgrade v2.24 because {table_name}.{column_name} "
...
def _validate_sequence_ranges(bind: sa.engine.Connection) -> None:
    for table_name, column_name in (
        *(("sessions", column_name) for column_name in _SESSION_SEQUENCE_COLUMNS),
        ("timeline_items", "updated_seq"),
    ):
