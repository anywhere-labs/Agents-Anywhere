...
- **设备配对。** 支持从 Web 端生成 token 启动命令，也支持 Connector 端 `pair` 输出配对码。
...
| 平台 / 入口 | 当前状态 | 说明 |
| --- | --- | --- |
| Web 控制台 | 当前主要支持客户端 | 目前以桌面浏览器为主要目标；移动浏览器也可用于状态查看、审批、设备管理和轻量 Session 控制。 |
| Connector CLI | 主要 Runtime 桥接层 | 运行在拥有工作区和本地 Agent Runtime 的机器上。具体能力取决于本机 shell、文件系统和已安装的 Codex / Claude 工具链。 |
| iOS | 原生客户端开发中 | 正在面向移动端 Session/Device 工作流和轻量控制场景开发。 |
| Android | 原生客户端开发中 | 正在面向移动端 Session/Device 工作流和轻量控制场景开发。 |
| macOS 桌面端 | Coming soon | Web 与 Connector 流程稳定后计划提供原生桌面封装。 |
| Windows 桌面端 | Coming soon | Web 与 Connector 流程稳定后计划提供原生桌面封装。 |

当前仓库已包含 Web 前端、FastAPI 后端、Connector CLI，以及开发中的原生移动端代码。Web 控制台仍是当前的参考客户端，原生客户端会随着实现成熟继续完善。
...
flowchart LR
    ...
    Connector["Connector<br/>本地守护进程 + CLI"]
...
- `postgres-next`：PostgreSQL 17，使用持久化 Docker volume。
- `server-next`：FastAPI 后端，发布在宿主机 `5174` 端口；它同源托管静态导出的 `web-next` UI，并处理 API / WebSocket 路径。
...
服务地址可以是不带 scheme 的域名、带端口的域名，或完整 URL。省略 `http` / `https` 时，CLI 会先尝试 HTTPS，再尝试 HTTP。
