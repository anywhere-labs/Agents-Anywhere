...
[Docker Quickstart](#quickstartdocker-启动完整应用) · [首次使用](#首次使用流程) · [Android APK](https://github.com/anywhere-labs/Agents-Anywhere/releases/tag/v0.1.0) · [Connector App](https://github.com/anywhere-labs/Agents-Anywhere/releases) · [自托管](#self-host生产风格部署) · [English](README.md)

![Agents Anywhere 会话工作台](docs/screenshots/hero.png)

> 截图来自正在快速迭代的产品界面，UI 细节可能随版本快速变化，请以实际运行效果为准。
...
- Web 控制台和 Android App 连接后端，让你和 Session 对话、接管任务、处理审批、浏览文件、打开终端。
...
Agents Anywhere 把这些长跑任务变成一个可以随时打开的工作台：看状态、看文件、看输出、批操作、打断、继续、切换设备，都可以在 Web 控制台或 Android App 里完成。
...
| Runtime | 厂商 | 当前状态 | 说明 |
| --- | --- | --- | --- |
| Codex | OpenAI | 当前适配最好 | 已支持大部分核心能力，包括 Runtime 发现、Session 同步、timeline 更新、审批、打断/接管、文件访问、shell task、交互式 terminal 和 Runtime 设置。部分厂商特有能力和附加功能仍在完善中。 |
| Claude Code | Anthropic | 基础功能已支持 | 当前代码已支持发现和基础 Session/控制流程。与 Codex 的深度能力对齐、更完整的历史/timeline 处理，以及高级 Runtime 行为仍在继续完善。 |
| Cursor | Anysphere | Coming soon | 暂未提供可用 adapter。 |
| OpenCode | SST | Coming soon | 暂未提供可用 adapter。 |
| Gemini CLI | Google | Coming soon | 暂未提供可用 adapter。 |
...
| 平台 / 入口 | 当前状态 | 说明 |
| --- | --- | --- |
| Web 控制台 | 当前主要支持客户端 | 完整的浏览器控制台，支持 Session、Device、审批、文件、终端、Runtime 设置、团队/管理员管理和 Session 详情。 |
| Android | 当前主要支持客户端 | 可从 [GitHub Release v0.1.0](https://github.com/anywhere-labs/Agents-Anywhere/releases/tag/v0.1.0) 下载 APK。Android App 是正式可用的原生客户端，支持 Session、Device、审批、文件、终端和移动端控制工作流。 |
| iOS | 原生客户端开发中 | 正在面向移动端 Session/Device 工作流开发。 |
| Windows / macOS Connector App | 已可用 | 可从 [GitHub Releases](https://github.com/anywhere-labs/Agents-Anywhere/releases) 下载桌面 Connector App。它会把本机 Connector 作为后台常驻进程运行，并提供配对、日志、托盘和开机启动控制。 |
| Linux Connector CLI | 已可用 | 使用 `connector/` 里的 Python CLI 或 `uvx anywhere-cli`。这是 Linux 服务器、开发机和 headless 环境的推荐 Connector 方案。 |
...
Docker stack 或 server 启动后，先从 Web 控制台开始。首次 setup token 流程只能在 Web 端完成。

1. 打开 Web 控制台，填入服务端日志里的 bootstrap token，创建第一个账号。第一个账号会成为管理员。
2. 在 Web 控制台里添加第一台 Device。这个 Device 是实际拥有代码工作区和本地 Codex / Claude Runtime 的机器。
3. 根据这台机器的系统选择 Connector：

| 目标机器 | 推荐 Connector | 说明 |
| --- | --- | --- |
| Windows | Agents Anywhere Connector App | 从 [GitHub Releases](https://github.com/anywhere-labs/Agents-Anywhere/releases) 下载桌面 Connector App。App 会让 Connector 作为后台进程运行，并引导完成配对。 |
| macOS | Agents Anywhere Connector App | 从 [GitHub Releases](https://github.com/anywhere-labs/Agents-Anywhere/releases) 下载桌面 Connector App。App 会让 Connector 作为后台进程运行，并引导完成配对。 |
| Linux | Connector CLI | 使用 `uvx anywhere-cli` 或 `connector/` 里的 CLI。Web UI 和 CLI 配对流程会给出具体命令或 pairing code。 |

Web 和 Android 端已经提供了完整的配对 UI，所以 README 只需要说明平台选择。第一台 Device 在线后，你可以用 Web 控制台或 Android App 作为日常入口：和 Session 对话、管理 Device、处理审批、浏览远程文件、打开终端，以及继续配对更多 Device。

高级 Connector 命令、Docker 化 Connector、带 SSH 的开发容器和 agent installer 镜像，见 [connector/README.md](connector/README.md) 和 [docker/README.md](docker/README.md)。
