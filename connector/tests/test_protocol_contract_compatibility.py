...
@pytest.mark.parametrize(
    ...
    sorted(
        path
        for slug in FIXTURE_MODELS
        for path in (CONTRACT_DIR / "fixtures" / "valid" / slug).glob("*.json")
    ),
...
def test_connector_model_catalog_builder_matches_contract() -> None:
    ...
    assert payload["models"][0]["metadata"]["raw"]["vendorExtension"] == {
        "channel": "preview"
    }
...
def test_connector_permission_catalog_builder_matches_contract() -> None:
    ...
    assert payload["permissions"][0]["metadata"]["runtimeSettings"] == {
