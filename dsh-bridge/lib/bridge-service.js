...
import { SessionManager } from './runtime/sessions.js';
import { BridgeError } from './wire/errors.js';
import { PROTOCOL_VERSION, RUNTIME_ID, } from './wire/protocol.js';
import { LoopbackJsonRpcServer } from './wire/server.js';
import { arrayField, isRecord, limitField, objectField, optionalBooleanField, optionalStringField, stringField, } from './wire/validation.js';
...
export class AgentsAnywhereBridgeService extends Service {
    ...
    async [Service.init]() {
        ...
        const sessions = new SessionManager(this.ctx, this.metadata, catalogs, this.config.maxListLimit, this.config.maxCommandLimit, (method, params) => this.notify(method, params), () => undefined, () => undefined);
        const interactions = new InteractionManager(this.ctx, this.config.maxPendingInteractions, agent => sessions.controllerForAgent(agent), notice => this.notify('notice.upsert', notice), () => undefined);
    ...
    async request(frame) {
        ...
        try {
            return await withTimeout(this.dispatch(frame.method, frame.params, controller.signal), timeoutMs, controller);
        }
        finally {
    ...
    async disconnected(reason) {
        for (const controller of this.activeRequests.values())
            controller.abort(new Error(`bridge disconnected: ${reason}`));
    ...
    async dispatch(method, params, signal) {
        ...
        switch (method) {
            ...
            case 'session.list':
                return await sessions.listSessions(limitField(params, 'limit', Math.min(100, this.config.maxListLimit), this.config.maxListLimit), optionalStringField(params, 'cursor'), optionalBooleanField(params, 'force') ?? false, signal);
            case 'session.getSnapshot': {
                const pair = sessionPair(params);
                return await sessions.snapshot(pair.sessionId, pair.externalSessionId, nonNegativeInteger(params, 'fromSeq', 0), limitField(params, 'limit', this.config.maxListLimit, this.config.maxListLimit), signal);
            }
            ...
            case 'session.listCommands': {
                const pair = sessionPair(params);
                return await sessions.listCommands(pair.sessionId, pair.externalSessionId, optionalStringField(params, 'query'), limitField(params, 'limit', Math.min(100, this.config.maxCommandLimit), this.config.maxCommandLimit), signal);
            }
            case 'session.executeCommand': {
                const pair = sessionPair(params);
                return await sessions.executeCommand(pair.sessionId, pair.externalSessionId, stringField(params, 'command'), optionalStringField(params, 'raw'), optionalStringField(params, 'args') ?? '', signal);
            }
            case 'session.respondInteraction': {
                const pair = sessionPair(params);
                return await sessions.respondInteraction(pair.sessionId, pair.externalSessionId, stringField(params, 'noticeId'), stringField(params, 'actionId'), params.inputData, signal);
            }
            case 'ping':
    ...
    async publishCatalogs(snapshot) {
        if (!this.initialized)
            return;
    ...
    notify(method, params) {
        ...
        if (endpoint === undefined)
            throw new Error('agents-anywhere bridge endpoint is not initialized');
        return endpoint.notify(method, params).catch((error) => {
    ...
    shutdownCore(reason) {
        ...
        for (const controller of this.activeRequests.values())
            controller.abort(new Error(`bridge shutdown: ${reason}`));
        const operation = (async () => {
            await this.interactions?.cancelAll();
            const result = await withDeadline(this.sessions?.shutdown() ?? Promise.resolve({ disposedSessions: 0, failedSessions: 0 }), this.config.shutdownTimeoutMs).catch(() => ({ disposedSessions: 0, failedSessions: 1 }));
    ...
    requireCatalogs() {
        if (this.catalogs === undefined)
            throw new BridgeError('DSH_SERVICE_UNAVAILABLE', 'Catalog services are not initialized.', { retryable: true });
        return this.catalogs;
    }
    requireSessions() {
        if (this.sessions === undefined)
            throw new BridgeError('DSH_SERVICE_UNAVAILABLE', 'Session services are not initialized.', { retryable: true });
        return this.sessions;
    }
}
function runtimeCapabilitiesPayload() {
    return {
        runtime: RUNTIME_ID,
...
function modelCatalogPayload(revision, models) {
    return {
        runtime: RUNTIME_ID,
        revision,
...
function permissionCatalogPayload(revision, permissions) {
    return {
        runtime: RUNTIME_ID,
        revision,
...
function sessionPair(params) {
    return {
        sessionId: stringField(params, 'sessionId'),
        externalSessionId: stringField(params, 'externalSessionId'),
    };
}
function selectionsField(params) {
...
function requiredSelections(params) {
    const selections = selectionsField(params);
...
function nonNegativeInteger(params, key, fallback) {
