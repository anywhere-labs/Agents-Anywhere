...
export class CatalogManager {
    ...
    async resolvePermission(id) {
        const preset = decodePermissionSelectionId(id);
        const snapshot = await this.current();
        if (!snapshot.permissions.some(item => item.selectionId === id && item.enabled))
            throw invalidSelection('permission');
        return preset;
    }
    /** Encode the effective permission of an existing Session log. */
    permissionFor(events) {
    ...
    async build() {
        ...
        const permissions = this.ctx.permissionPresets.names.map((preset) => {
...
function invalidSelection(kind) {
