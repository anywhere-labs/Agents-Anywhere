...
export type BridgeMuxPayload =
  ...
  | {
      type: 'session/queue'
      sessionId: SessionId
      items: Array<{ id: Message['id']; placement: 'queued' | 'steering' | 'context'; message: Message }>
    }
  | {
...
export type BridgeRpcReceipt = { accepted: true } | { accepted: false; reason: 'not-pending' | 'bad-response' }

/** Assign one push-only mux correlation id. */
export function bridgeMuxEnvelope(payload: BridgeMuxPayload): BridgeMuxEnvelope {
  return { rpcId: randomUUID(), payload }
}

/** Assign one push-only Host correlation id. */
export function bridgeHostEnvelope(payload: BridgeHostPayload): BridgeHostEnvelope {
  return { rpcId: randomUUID(), payload }
}
