...
public final class RemoteTerminalView extends View {
    ...
    public RemoteTerminalController mTermSession;
    ...
    public RemoteTerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public boolean onSingleTapUp(MotionEvent event) {
                ...
                requestFocus();
                debug("singleTap focus=" + hasFocus());
                mClient.onSingleTapUp(event);
                ((InputMethodManager) getContext().getSystemService(Context.INPUT_METHOD_SERVICE))
                    .showSoftInput(RemoteTerminalView.this, InputMethodManager.SHOW_IMPLICIT);
                return true;
            }
    ...
    /**
     * @param client The {@link RemoteTerminalViewClient} interface implementation to allow
     *                           for communication between {@link TerminalView} and its client.
     */
    public void setRemoteTerminalViewClient(RemoteTerminalViewClient client) {
        this.mClient = client;
    }
    ...
    /**
     * Attach a {@link RemoteTerminalController} to this view.
     *
     * @param session The {@link RemoteTerminalController} this view will be displaying.
     */
    public boolean attachSession(RemoteTerminalController session) {
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        debug("createInputConnection inputType=" + outAttrs.inputType + " imeOptions=" + outAttrs.imeOptions + " focus=" + hasFocus());
    ...
    public void onScreenUpdated(boolean skipScrolling) {
        ...
        mScreenUpdates++;
        if (shouldLog(mScreenUpdates)) {
            debug("screenUpdated #" + mScreenUpdates + " skip=" + skipScrolling + " topRow=" + mTopRow + " rows=" + mEmulator.mRows + " cols=" + mEmulator.mColumns);
        }
    ...
    public void updateSize() {
        ...
        if (mEmulator == null || (newColumns != mEmulator.mColumns || newRows != mEmulator.mRows)) {
            debug("updateSize cols=" + newColumns + " rows=" + newRows + " view=" + viewWidth + "x" + viewHeight);
    ...
    protected void onDraw(Canvas canvas) {
        mDraws++;
        if (shouldLog(mDraws)) {
            debug("draw #" + mDraws + " emulator=" + (mEmulator != null));
        }
        if (mEmulator == null) {
    ...
    public RemoteTerminalController getCurrentSession() {
        return mTermSession;
    }
     ...
     * {@link RemoteTerminalViewClient#onEmulatorSet()} event after calling {@link #attachSession(RemoteTerminalController)}
     ...
     * call this directly after {@link #attachSession(RemoteTerminalController)} since {@link #updateSize()}
     ...
     * {@link com.termux.terminal.RemoteTerminalControllerClient#onTerminalCursorStateChange(boolean)}
     ...
     * before calling this.
     *
     * It should also be called after terminal is reset with {@link RemoteTerminalController#reset()} in case
