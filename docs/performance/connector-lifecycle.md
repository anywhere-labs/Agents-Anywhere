...
首次初始化也支持多服务进程：配置 Redis 后，各 worker 共用同一个有有效期的初始化令牌，避免从一个进程日志取得的令牌被另一个进程拒绝。创建管理员后清理共享令牌，数据库中的首次管理员保护继续负责防止重复初始化。Redis 不可用时拒绝初始化验证，不回退到进程本地令牌；未配置 Redis 的单进程部署保留原有行为。
