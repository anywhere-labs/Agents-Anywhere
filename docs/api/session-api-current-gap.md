...
| SessionMeta read | `GET /api/v2/sessions/{sessionId}/meta` | missing | Add after frontend call sites are ready to consume split meta. |
| SessionMeta update | `PATCH /api/v2/sessions/{sessionId}/meta` | missing | Add or move current `PATCH /sessions/{sessionId}` semantics here. |
...
| SessionTimeline read | `GET /api/v2/sessions/{sessionId}/timeline` | missing | Add. This should return durable timeline only. |
