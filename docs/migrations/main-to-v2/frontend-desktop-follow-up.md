...
- `SessionToolSidebarStateProvider` 按账号隔离内存状态，工具宿主在页面切换时
  ...
  临时会话 ID 到服务端 ID 的迁移；刷新网页后不承诺恢复终端或未保存编辑。
...
- 终端使用现有浏览器 WebSocket 与 Connector 的普通终端生命周期，不接入
  Electron 的持久化租约、退出钩子或本机 Connector 识别。显式关闭标签调用
  终端关闭 API；创建请求返回前标签已关闭时也清理返回的终端。账号退出时
  清理已知终端，意外关闭页面后的清理由现有 Connector 机制处理。
...
- 工具侧栏为空时，审阅、终端、文件使用三张纵向排列的小卡片，整体最大
  宽度 256px、卡片间距 8px。每张卡片包含 16px 图标、14px 标题与 12px
  简短说明，复用现有 Card 的圆角与表面样式；整张卡片支持点击和键盘打开。
