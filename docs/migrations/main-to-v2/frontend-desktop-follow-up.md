...
- 增加 `POST /api/v2/projects/resolve`，复用服务端的工作目录归属逻辑；查到
  已有项目时保留其 ID、名称和手动创建标记，新建则保持自动项目身份。
  该接口校验设备归属、撤销状态与绝对路径，并在并发唯一性冲突后重新查询。
  会话请求仍必须携带 `projectId`，无需数据库迁移。Web 与 Server 应一起更新。
- 自动解析失败时保留任务草稿，重复发送不会重复创建。文件选择器的加载状态
  和请求序号保护确认路径，失败、加载中或文件目标均不能提交为工作目录。

验证通过 Web 全部 178 项测试（新增 6 项目录匹配、命名与项目解析测试）和
TypeScript 检查，Server 12 项项目唯一性、并发解析、归属与显示规则测试。
Chrome/WebKit 通过 12 组中英文、深浅色、1440px/390px/320px 布局检查及
36 项交互检查，覆盖直接打开文件选择器、名称生成/修改、取消与提交、跨设备
项目选择、默认用户目录、自动解析、失败重试和模式切换。浏览器使用实际 Web
