...
test("AA archived sessions include every source state and ignore legacy aliases", () => {
  const archived = sourceStates.map((state) => session(state, true))
  const filter = { ...defaultFilter, status: "archived" }
  assert.deepEqual(filterSessions(archived, filter, "").map((item) => item.id), sourceStates)
  assert.deepEqual(filterSessions([session("archived")], filter, ""), [])
})
