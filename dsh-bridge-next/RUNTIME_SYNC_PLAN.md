...
本文件覆盖三项一起交付的修改：官方侧栏规则过滤会话、丢弃内部 notice、以首次同步和事件推送替代 DSH 定时扫描。用户确认本方案之后，才修改运行代码、协议和数据库。
...
- 这期仍为只读接入。不实现从 AA 发消息、新建 DSH 会话、切换模型、审批应答等写操作；会实时发现用户在 DSH 中创建的会话。
...
| F：联调验收 | 真实官方服务组合、两个进程协议、Server 数据库与客户端更新 | 下表全部通过，提交仅含任务文件 |
...
除此之外无需再扩展产品流程。用户点头后，按以上 A–F 顺序完成三项代码修改和验证。
