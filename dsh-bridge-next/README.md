...
Runtime 已实现 DSH 一键配置、官方侧栏过滤、原生会话和历史读取、首次完整校准、明确归档事件同步及详情/发送前检查、实时事件同步、纯文本新建/续聊及中断，以及 `ask_user_question` 问答。项目沿用后端统一的 CWD 分类和末段命名，不同步 DSH 项目名称或分组。内部 notice 不进入 Timeline，DSH 不再定时扫描历史；连接恢复后用现有后端完整替换接口校准。附件、模型/权限目录和工具权限审批应答暂未开放。实现与消息映射见 [会话读取](./RUNTIME_READS.md)、[事件同步方案](./RUNTIME_SYNC_PLAN.md)和[用户问答](./USER_QUESTIONS.md)。
