...
Desktop 与插件通过固定的 `<操作系统用户主目录>/.agentsanywhere/machine.json` 共享安装位置和本机 Connector ID 历史。Desktop 每次启动检查实际路径，内容正确时不重写；只在新建本机设备时按顺序追加 ID，普通重连不追加。插件每次检测都重新读取；可执行文件已不存在时保留历史 ID 并允许 Web 流程，记录损坏或无权限时报告错误。旧 `desktop/install.json` 在新文件不存在时仍可读取。

OAuth 后插件获取当前用户的设备列表，与共享 ID 按本地记录顺序匹配；多个匹配取第一个，用现有 `/revoke` 接口换新 Connector token，随后上线并进入原 Web Agent 配置引导。旧插件私有绑定作为最后一个本机候选保留兼容；只有没有匹配才注册新设备。读取、列设备或重连失败都不会降级为新建。普通恢复已有有效 token 时不重复轮换。共享文件不含凭据，详见[本机共享记录契约](../contracts/local-machine/1.0/README.md)。

安装未首启时的补查、协议唤起和 Desktop onboarding 仍属于后续工作；检测到有效 Desktop 安装时，插件继续遵守由 Desktop 管理 Connector 的边界。
