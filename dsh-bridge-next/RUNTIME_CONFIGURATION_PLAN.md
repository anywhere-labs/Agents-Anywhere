...
更新日期：2026-09-07。状态：业务方案，待实施。本轮仅整理文档。
...
- **额外的更强要求：AA 操作连 DSH 自己的全局默认也完全不能改变。**现有官方 `selectModel` 无法直接保证这一点。如果需要这个隔离级别，需要官方提供会话级、不保存默认的入口，或验证完整的公开底层控制方案后另行确定。

本方案优先满足“立即调用官方切换接口”和“AA 偏好独立”的已确认规则，不声称已经解决 DSH 全局默认零写入。不能采用切换后恢复全局设置、访问私有 `WeakMap`、修改 DSH 文件等方式伪装隔离，这会与 DSH 本地操作及其他会话竞争。

仅追加一条 `model/selection` 日志，也不能视为已经切换了官方控制器持有的活跃 Agent。必须验证后续真实请求使用了新值。
