...
export function MobileConnection({ t, host, disabled }: { t: Translate; host: OnboardingHostApi; disabled: boolean }) {
  ...
  const message = status === 'pending_web_confirm' ? t('{name}请求连接此账号', { name: qr?.deviceName || t('一台手机') })
    : status === 'approved' ? t('已确认，正在完成手机登录…')
      : status === 'consumed' ? t('手机已连接')
        : status === 'rejected' ? t('已拒绝此次连接')
          : status === 'expired' ? t('二维码已过期') : t('使用手机端扫描二维码')
  return <div className={css.connection}>
    <Button variant="outline" className={css.button} icon={<Smartphone size={16} />}
      ...
      {expanded ? t('收起二维码') : t('手机连接')}
    </Button>
    {expanded ? <div id={id} className={css.content} aria-busy={busy}>
      {busy ? <p className={css.status} role="status"><StateDot state="ongoing" />{t('正在处理…')}</p> : <>
        {qr?.qrImage && status === 'pending_scan' ? <img className={css.qr} src={qr.qrImage} width={256} height={256} alt={t('手机连接二维码')} /> : null}
        ...
        {status === 'pending_scan' ? <p className={css.hint}>{t('有效至 {time}', { time: new Date(qr!.expiresAt).toLocaleTimeString(t('locale.code'), { hour: '2-digit', minute: '2-digit' }) })}</p> : null}
        {status === 'pending_web_confirm' ? <div className={css.actions}>
          <Button variant="outline" disabled={disabled} onClick={() => void run(() => host.confirmMobileLogin(qr!.id, false))}>{t('拒绝连接')}</Button>
          <Button variant="primary" disabled={disabled} onClick={() => void run(() => host.confirmMobileLogin(qr!.id, true))}>{t('确认连接')}</Button>
        </div> : null}
      </>}
      ...
      {!busy && (!qr || status === 'expired' || status === 'rejected' || status === 'consumed' || error) ?
        <Button variant="ghost" disabled={disabled} onClick={generate}>{status === 'consumed' ? t('连接另一台手机') : t('重新生成二维码')}</Button> : null}
    </div> : null}
  </div>
}
