...
下一项拟实施工作为：按官方侧栏过滤会话、丢弃内部 notice，以及“首次同步 + 实时事件推送 + 断线补偿”。详细方案见 [Runtime 过滤与事件同步计划](./RUNTIME_SYNC_PLAN.md)。**该方案待用户确认，本轮只编写文档；确认后再一起修改三项代码。** 模型/权限目录、新建和发送等写能力排在这项之后。
...
5. **DSH 业务迁移与端到端验证**：第一阶段发现与读取已实现并通过真实官方服务组合和 Python 跨进程验证；下一阶段接入模型/权限目录、新建会话与发送、实时事件和中断，再处理审批和提问。
