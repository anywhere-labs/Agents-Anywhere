...
未安装 Desktop 时，弹窗顶部提供「登录和连接」「设置」两个标签页。登录和连接页保留登录流程；登录后显示头像、账号、Connector 运行状态和「打开 Web」「手机连接」「退出登录」。手机连接在按钮下方直接显示二维码，接入现有手机授权、确认/拒绝、过期刷新与成功状态，不展示 App 安装地址。Web 按钮直接进入应用，退出登录负责停止本机连接并清理用户凭据。

设置页复用 Desktop「此 Desktop」的管理内容：设备 ID、服务器、uv 路径与解析结果、PyPI 镜像、同步间隔、启动/停止/重启、数据和日志目录、重置连接。补充插件启动时自动恢复开关及可展开的心跳/重连/已有会话同步参数。设置单独持久化，运行参数保存后重启正在运行的 Connector，已停止时仅保存。重置先撤销当前设备凭据；撤销失败保留本地状态，只有再次确认才清理本地数据。已有 Desktop 的占位 UI 保持原流程，所有新增管理接口都检查本机管理归属。
