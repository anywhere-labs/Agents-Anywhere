...
2. **插件骨架**：核实目标 DSH 版本的实际接口，实现加载、设置入口、本机端点和握手；验证卸载后的资源清理。
3. **Agent 功能迁移**：逐项实现会话、模型、消息、Timeline 和交互。每迁入一项，就调整 Python 对应方法为薄转发，并验证整条链路。
4. **管理功能接入**：完善 Connector 启停、环境检测、日志、账号登录和设备绑定，接入设置页。
5. **完整验收与切换**：验证收发消息、中断、审批、历史与实时一致性、断线恢复及重启后的状态；通过后再安排旧插件替换。
