...
export async function patchSession(
  ...
  if (idx !== -1) Object.assign(mockSessions[idx], patch)
  return { session: mockSessions[idx] }
}
...
export async function pollConnectorOnline(
  ...
  const count = ((globalThis as Record<string, number>)[key] ?? 0) + 1;
  (globalThis as Record<string, number>)[key] = count
  if (count >= 3) {
...
export async function claimConnector(
  ...
  const key = `__poll_${connectorId}`;
  (globalThis as Record<string, number>)[key] = 0
  return { ok: true }
}
