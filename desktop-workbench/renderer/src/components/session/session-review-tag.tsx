...
export function SessionReviewTag({
  ...
  return (
    <div className="pointer-events-none flex justify-center px-4 pt-2">
      <Badge
        variant="outline"
        className="pointer-events-auto h-auto max-w-full gap-2 rounded-full bg-card/85 py-1 pr-1 pl-3 shadow-sm backdrop-blur-xl supports-backdrop-filter:bg-card/70"
      >
        <span className="truncate" aria-live="polite" aria-atomic="true">
          {t("reviewFilesChanged", { count: fileCount })}
        </span>
        <Button type="button" variant="ghost" size="xs" className="rounded-full" onClick={onReview}>
          {t("review")}
        </Button>
      </Badge>
    </div>
