...
type SidebarContextProps = {
  ...
  open: boolean
  setOpen: (open: boolean | ((open: boolean) => boolean), options?: { persist?: boolean }) => void
  openMobile: boolean
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean), options?: { persist?: boolean }) => {
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        ...
        className={cn(
          "group/sidebar-wrapper flex h-svh min-h-0 w-full overflow-hidden overscroll-none has-data-[variant=inset]:bg-sidebar",
          className
        )}
        {...props}
      >
...
function Sidebar({
  ...
  if (contained) {
    return (
      <div
        className="group h-full min-h-0 min-w-0 text-sidebar-foreground"
        data-state={state}
      ...
      >
        ...
        >
          <div
            ...
            className="flex size-full min-w-0 flex-col bg-sidebar group-data-[variant=floating]:rounded-2xl group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
          >
            {children}
          </div>
        </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  return (
    <button
      ...
      tabIndex={-1}
      className={cn(
        "pointer-events-none absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
