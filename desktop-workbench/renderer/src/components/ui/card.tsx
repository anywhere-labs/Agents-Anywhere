...
function Card({
  ...
  variant = "default",
  ...props
}: React.ComponentProps<"div"> & { size?: "default" | "sm"; variant?: "default" | "settings" }) {
  return (
    <div
      data-slot="card"
      data-size={size}
      ...
      className={cn(
        variant === "settings"
          ? "group/card flex flex-col gap-0 overflow-hidden rounded-xl border border-border bg-card py-0 text-sm text-card-foreground [--card-spacing:--spacing(6)]"
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-heading text-base font-medium group-data-[variant=settings]/card:font-semibold", className)}
      {...props}
    />
  )
}
